...
public:
    ...
    void update(float dt) override {
        if (!m_running || isFinished()) return;
        m_elapsed += dt;
        float t = std::min(m_elapsed / m_dur, 1.f);
        *m_target = doLerp(m_from, m_to, m_ease(t));
        if (t >= 1.f && m_onComplete) m_onComplete();
    }
    bool isFinished() const override { return m_elapsed >= m_dur; }
    void reset() override { m_elapsed = 0; *m_target = m_from; }
...
public:
    ...
    void setImmediate(T v) { if (m_anim) m_anim->stop(); m_val = m_target = v; }
